...
# ---------- tiny logger (prints to Vercel logs) ----------
def log(*args):
    try:
...
# Boot log (never prints secrets)
log("BOOT env:",
    "TOKEN_SET" if bool(BOT_TOKEN) else "TOKEN_MISSING",
    "OPENAI_SET" if bool(OPENAI_API_KEY) else "OPENAI_MISSING",
    "REDIS_URL_SET" if bool(REDIS_URL) else "REDIS_URL_MISSING",
    "REDIS_TOKEN_SET" if bool(REDIS_TOKEN) else "REDIS_TOKEN_MISSING",
)
...
def rget(key):
    ...
    try:
        r = requests.get(f"{REDIS_URL}/get/{key}", headers={"Authorization": f"Bearer {REDIS_TOKEN}"}, timeout=8)
        return r.json().get("result") if r.status_code == 200 else None
    except Exception as e:
        log("rget error:", repr(e)); return None

def rsetex(key, seconds, value):
    if not REDIS_URL or not REDIS_TOKEN: return
    try:
        requests.get(f"{REDIS_URL}/setex/{key}/{seconds}/{value}", headers={"Authorization": f"Bearer {REDIS_TOKEN}"}, timeout=8)
    except Exception as e:
        log("rsetex error:", repr(e))

def rincr(key):
    if not REDIS_URL or not REDIS_TOKEN: return
    try:
        requests.get(f"{REDIS_URL}/incr/{key}", headers={"Authorization": f"Bearer {REDIS_TOKEN}"}, timeout=8)
    except Exception as e:
        log("rincr error:", repr(e))

def rset(key, value):
    if not REDIS_URL or not REDIS_TOKEN: return
    try:
        requests.get(f"{REDIS_URL}/set/{key}/{value}", headers={"Authorization": f"Bearer {REDIS_TOKEN}"}, timeout=8)
    except Exception as e:
        log("rset error:", repr(e))
...
class handler(BaseHTTPRequestHandler):
    def do_POST(self):
        try:
            ...
            if not msg:
                log("No message in update:", update); return self._ok()
            ...
            log("FATAL do_POST:", traceback.format_exc())
            self._ok()  # prevent Telegram retry storm
